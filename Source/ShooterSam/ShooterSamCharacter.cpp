...
void AShooterSamCharacter::BeginPlay()
{
	...
	// Hide mannequin skeleton's default weapon bone
	...
	if (World && GunClass)
	{
		...
		{
			Gun->SetOwner(this);
			// Attach to skeleton socket defined in mannequin mesh
...
void AShooterSamCharacter::OnDamageTaken(AActor* DamagedActor, float Damage, const UDamageType* DamageType, AController* InstigatedBy, AActor* DamageCauser)
{
	if (bIsAlive)
	{
		...
		if (Health <= 0.0f)
		{
			...
			// Check if this is player or AI death BEFORE detaching controller
			AShooterSamGameMode* GameMode = GetWorld() ? GetWorld()->GetAuthGameMode<AShooterSamGameMode>() : nullptr;
			bool bIsPlayer = Cast<APlayerController>(GetController()) != nullptr;
			
			// Disable collision to prevent blocking and allow ragdoll
			GetCapsuleComponent()->SetCollisionEnabled(ECollisionEnabled::NoCollision);
			// Release controller for respawn/spectate handling
			...
			if (GameMode)
			{
				if (bIsPlayer)
				{
					// Player died - trigger lose condition
